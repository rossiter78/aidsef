...
- **`/aidsef-begin` onboarding skill** — automate new-project setup (getting-started Phase D steps 4–9): delete the framework-authoring files, draft the project `README.md`/`CLAUDE.md` from a short interview, walk the user through the two 👤 browser steps (OAuth secret, App access), run `setup-repo.sh`, then prompt for the few real settings (operating profile, any threshold tuning, any project-specific law) and open the bootstrap and ratification PRs. Found while running dogfood project #1's setup by hand — the steps existed only as chat guidance, which is exactly what a skill is for. (Supersedes the earlier "post-clone bootstrap script" idea: a skill can interview; a script can't.)
